...
public class SoMeSPCIntegrator {
		...
		try
		{
		   ...
		   //Aguarda 1s antes de agendar cada job para evitar problemas de concorrência.
			...
			if (item.getNomeFerramentaColetora().equalsIgnoreCase("Taiga")){
				...
				System.err.println("Não foi possível agendar os jobs do Taiga pois a ferramenta informada foi outra: " + item.getNomeFerramentaColetora());
			}
	...
	public static synchronized PlanoDeMedicaoDoProjeto criarPlanoMedicaoProjetoSonarQubeSoMeSPC(List<ItemPlanoDeMedicaoDTO> itemPlanoDeMedicaoDTO, 
			Periodicidade periodicidade, SonarLoginDTO sonarLogin, Recurso recurso) throws Exception {
	...
	plano.setNome("Plano de Medição do Projeto " + proj.getNome() + " (Wizard) - " + dataHora);
	plano.setVersao("1");
	plano.setDescricao("Plano de Medição do Projeto criado via wizard em " + dataHora);
	plano.setProjeto(proj);
	...
	for(ItemPlanoDeMedicaoDTO item: itemPlanoDeMedicaoDTO){
		...
		MedidasSonarQube medidasSonar = MedidasSonarQube.get(item.getMedida());		
		sonarIntegrator.criarMedidasSoMeSPC(Arrays.asList(medidasSonar));
						
	    String queryMedida = String.format("SELECT p FROM Medida p WHERE p.nome='%s'", item.getMedida());
	    TypedQuery<Medida> typedQueryMedida = manager.createQuery(queryMedida, Medida.class);
	    Medida med = typedQueryMedida.getSingleResult();

	    String queryDefMedida = "SELECT p FROM DefinicaoOperacionalDeMedida p WHERE p.nome='Definição operacional da medida do SonarQube - " + item.getMedida() + "'";
	    TypedQuery<DefinicaoOperacionalDeMedida> typedQueryDefMedida = manager.createQuery(queryDefMedida, DefinicaoOperacionalDeMedida.class);
	    DefinicaoOperacionalDeMedida defMed = typedQueryDefMedida.getSingleResult();
		...
		try
		{
		   ...
		   //Aguarda 1s antes de agendar cada job para evitar problemas de concorrência.
			...
			if (item.getNomeFerramentaColetora().equalsIgnoreCase("SonarQube")){
				sonarIntegrator.agendarSonarQubeMedicaoJob(plano, recurso.getChave(), medida, periodicidade, sonarLogin); 
			} else {
				System.err.println("Não foi possível agendar os jobs do SonarQube pois a ferramenta informada foi outra: " + item.getNomeFerramentaColetora());
			}
