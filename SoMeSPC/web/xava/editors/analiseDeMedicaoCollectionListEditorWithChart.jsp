...
<%-- <% 
	...
	{
		AnaliseDeMedicao analiseDeMedicao = (AnaliseDeMedicao) MapFacade.findEntity("AnaliseDeMedicao", key2);

		//TODO:
		//se escala da medida do plano for numerica
		...
		if(analiseDeMedicao.getMedicao().size() > 0)
		{
			...
			{
				datas += "'" + medicao.getData().toString() + "', ";
				valores += medicao.getValorMedido().getValorMedido() + ", ";
			}
		...
		function() {
			if(valores){
				if(document.getElementById('canvasGrafico'))
				{
			        ...
			        line.Set('chart.tooltips', <%= valores %>);
